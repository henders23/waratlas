...
import { phaseAt, type AtlasEvent, type WarData } from './data/war';
...
export function App({ war, onSwitch }: { war: WarData; onSwitch: (id: string | null) => void }) {
  ...
  const byId = useMemo(() => new Map(war.events.map((e) => [e.id, e])), [war]);
  const selEv = selected ? byId.get(selected) : undefined;

  // Map lifecycle.
  useEffect(() => {
    document.title = `${war.title}, ${war.subtitle.replace(/ – /g, '–')} · Atlas of Wars`;
    const base = `${import.meta.env.BASE_URL}geo/${war.id}/`;
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      if (now - areaAt > 120 && atlas.current) {
        areaAt = now;
        setArea(atlas.current.focusArea(t));
      }
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => history.replaceState(history.state, '', `#${h}`), 250);
  ...
  return (
    <div className={`app${capture ? ' capture' : ''}${selEv || panel === 'chronicle' ? ' has-panel' : ''}${ready ? ' ready' : ''}`}>
      ...
      <Header war={war} onSwitch={onSwitch} />
      <Clock war={war} focusKm2={area} />
      <nav className="tools">
      ...
      {regionTip && !evTip && (
        <div className="tip region-tip" style={{ left: regionTip.x, top: regionTip.y }}>
          ...
          <span>
            ...
            {regionTip.info.status === 'contested' ? 'Contested — ' : regionTip.info.status === 'vassal' ? `${war.text.vassalTip} ` : ''}
          ...
          <div className="intro-card">
            ...
            <div className="intro-actions">
              <button className="primary" onClick={begin} disabled={!ready}>
                {ready ? war.text.intro.play : 'Loading the world…'}
              </button>
