...
import { isFocus, phaseAt, stepAt, type AtlasEvent, type Polity, type Status, type WarData } from '../data/war';
...
// Fades and blends are tuned for the Mongol atlas's 88 years and scaled by war.timeScale.
...
const APPEAR = 0.06; // years over which an event's pin fades in
const ACTIVE_LINGER = 0.35; // years an ended event keeps its pulse
const PIN_LINGER = 2.5; // years an ended event's badge takes to settle into a dot
...
export class AtlasMap {
  ...
  private cityMarkers: { marker: maplibregl.Marker; el: HTMLElement; rank: number; from: number; to: number }[] = [];
  private ownerKey = '';
  ...
  constructor(container: HTMLElement, war: WarData, geoBase: string) {
    ...
    this.map = new maplibregl.Map({
      container,
      style: baseStyle(),
      center: war.camera.center,
      zoom: war.camera.zoom - (small ? war.camera.smallZoomOffset : 0),
      minZoom: 1.2,
      maxZoom: 9,
  ...
  private async init(geoBase: string) {
    ...
    // Frontiers: kind 1 = the focus side against the world, 2 = within the focus side, 3 = between others.
    m.addLayer({
  ...
  private addCities() {
    for (const c of this.war.cities) {
      ...
      this.cityMarkers.push({ marker, el, rank: c.rank, from: c.from ?? -Infinity, to: c.to ?? Infinity });
    }
    this.onZoom();
  }
  ...
  private updateTerritory(t: number) {
    ...
    for (const f of this.regions) {
      ...
      const k = prev ? ease(clamp01((t - step.t) / (TRANSITION * this.war.timeScale))) : 1;
      ...
      const flash = fresh ? Math.pow(clamp01(1 - (t - step.t) / (FLASH * this.war.timeScale)), 2) * (t >= step.t ? 1 : 0) : 0;
  ...
  private updateFrontiers(owners: Map<string, { polity: string; status: Status }>) {
    ...
    for (const b of this.borders) {
      ...
      if (A && B && A.polity !== B.polity) {
        const ma = isFocus(P.get(A.polity));
        const mb = isFocus(P.get(B.polity));
  ...
  private updateLabels(owners: Map<string, { polity: string; status: Status }>) {
    ...
    for (const [pid, fs] of groups) {
      ...
      if (area < this.war.labelMinKm2[vw < 720 ? 1 : 0]) continue;
      ...
      const size = Math.max(9, Math.min(30, (5 + Math.sqrt(area) / 90) * this.war.labelScale) * scale);
  ...
  /** Hide smaller polity labels that would overlap a larger one on screen, and minor city names under any of them. */
  private declutter() {
    ...
    for (const c of this.cityMarkers) {
      if (c.rank !== 2 && window.innerWidth >= 720) continue;
      const r = (c.el.lastElementChild as HTMLElement).getBoundingClientRect();
      c.el.classList.toggle('crowded', r.width > 0 && placed.some((p) => r.left < p.right && r.right > p.left && r.top < p.bottom && r.bottom > p.top));
    }
  }
  ...
  private updateEvents(t: number) {
    ...
    const ts = this.war.timeScale;
    for (const e of this.war.events) {
      const appear = clamp01((t - e.t0 + APPEAR * ts) / (APPEAR * ts));
      let vis = 0;
      let active = 0;
      let past = 0;
      if (t >= e.t0 - APPEAR * ts) {
        if (t <= e.t1) {
          ...
          const since = t - e.t1;
          active = clamp01(1 - since / (ACTIVE_LINGER * ts));
          vis = clamp01(1 - since / (PIN_LINGER * ts)) * (0.5 + 0.5 * active);
  ...
  /** Area in km² of regions held by, or tributary to, the focus side at time t. */
  focusArea(t: number) {
    let km2 = 0;
    for (const f of this.regions) {
      ...
      if (isFocus(this.war.polities.get(step.polity)) && step.status !== 'contested') km2 += f.properties.areaKm2;
    }
    return km2;
  }
  ...
  flyToEvent(e: AtlasEvent) {
    const z = this.map.getZoom();
    const zoom = e.geometry === 'area' ? Math.max(3.4, Math.min(5.2, 8.6 - Math.log2(e.radiusKm ?? 200))) : Math.max(z, this.war.eventZoom);
    const small = window.innerWidth < 720;
    this.map.flyTo({
  ...
  flyToPhase(t: number, duration = 2600) {
    ...
    const small = window.innerWidth < 720;
    this.map.flyTo({ center: p.camera.center, zoom: p.camera.zoom - (small ? this.war.camera.smallZoomOffset : 0), duration, essential: true, curve: 1.2 });
  }
