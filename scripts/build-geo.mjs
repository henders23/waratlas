// Builds the atlas's static geography from Natural Earth (public domain):
//   public/geo/regions.json  – historical regions (polygons) with area and label point
//   public/geo/borders.json  – shared border arcs tagged with the regions on each side
//   public/geo/rivers.json, public/geo/lakes.json – physical context
...
for (const region of REGIONS) {
  for (const m of region.members) {
    ...
    const hits = adm1.filter((f) => f.properties.adm0_a3 === spec.a && (spec.n === undefined || f.properties.name === spec.n));
    if (!hits.length) {
