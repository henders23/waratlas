...
export default function App() {
  ...
  return <div className="site">
    ...
    {showAbout && <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) setShowAbout(false); }}><section ref={aboutRef} className="about-modal" role="dialog" aria-modal="true" aria-labelledby="about-title"><button className="modal-close" aria-label="Close" onClick={() => setShowAbout(false)}><X size={20}/></button><span className="eyebrow"><BookOpen size={14}/> A NOTE ON THE ATLAS</span><h2 id="about-title">History has <em>no hard edges.</em></h2><p>This atlas presents all {events.length} included records in the Mongol R72 denominator, from 1206 through its 1294 boundary. Five discovery rows are merged into included stories. One combined candidate was superseded by separately sourced city records.</p><p>The colored map regions are generalized interpretive graphics. They are not boundaries derived from the source pack. A point appears only when a named locality can be shown responsibly; many stories are intentionally unplotted.</p><p>Dates with ranges or uncertainty retain that wording in each story. The source notes show primary citations, map restrictions, and the one shortened display label.</p><button className="modal-action" onClick={() => setShowAbout(false)}>RETURN TO THE ATLAS <ArrowRight size={16}/></button></section></div>}
  </div>;
}
