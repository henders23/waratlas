...
export default function ArchiveDialog({ events, onSelect, onClose }: Props) {
  ...
  return <div className="modal-backdrop archive-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={root} className="archive-dialog" role="dialog" aria-modal="true" aria-labelledby="archive-title">
      <div className="archive-head"><div><span className="eyebrow">THE COMPLETE CHRONICLE / R72</span><h2 id="archive-title">Every turning point, <em>in view.</em></h2><p>All {events.length} included records in the Mongol denominator, 1206–1294. Some events have no precise map location.</p></div><button className="modal-close" onClick={onClose} aria-label="Close chronicle"><X size={20}/></button></div>
